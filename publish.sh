...
# Exclude this script itself: line 43 defines the patterns (literal SERVICE_ROLE,
# sk-, etc.), so scanning it self-matches and false-positives every publish.
HITS="$(git grep --cached -nIE "$PATTERNS" -- ':!publish.sh' 2>/dev/null || true)"
if [ -n "$HITS" ]; then
